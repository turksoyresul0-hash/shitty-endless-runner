(()=>{
  ...
  function showDamage(pos,amount,head=false,self=false){
    ...
    const d=document.createElement('div');d.className='dmg-number'+(head?' head':'')+(self?' self':'');d.textContent=(head?'HEADSHOT ':'')+Math.round(amount);d.style.left=((v.x*.5+.5)*innerWidth)+'px';d.style.top=((-v.y*.5+.5)*innerHeight)+'px';document.body.appendChild(d);setTimeout(()=>d.remove(),720);
    const c=document.querySelector('.cross');if(c&&!self){const old=c.textContent;c.textContent='✕';c.style.fontSize='30px';setTimeout(()=>{c.textContent=old;c.style.fontSize='26px'},90)}
  }
  ...
  function incoming(e){let m;try{m=JSON.parse(e.data)}catch{return}
    ...
    if(m.type==='respawn'){
      if(m.id!==myId){const g=findRemote(m.id);if(g){g.visible=true;g.position.set(m.x||0,m.y||2,m.z||12);g.userData.hp=100;g.userData.armor=100}}
      else{const p=window.__fpsGame?.player;if(p){p.p.set(m.x||0,m.y||2,m.z||12);p.hp=100;p.armor=100;p.vy=0;p.jump=true}window.__fpsDead=false;setLocalVitals(100,100);note('🔄 YENİDEN DOĞDUN')}
    }
  }
